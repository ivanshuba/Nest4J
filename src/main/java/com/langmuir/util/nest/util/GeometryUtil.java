...
import com.langmuir.util.nest.data.SegmentRelation;
...
public class GeometryUtil {
  ...
  public static Boolean pointInPolygon(Segment point, NestPath polygon) {
    ...
    for (int i = 0, j = polygon.size() - 1; i < polygon.size(); j = i++) {
      double xi = polygon.getSegment(i).x + offsetX;
      double yi = polygon.getSegment(i).y + offsetY;
      double xj = polygon.getSegment(j).x + offsetX;
      double yj = polygon.getSegment(j).y + offsetY;
      ...
      if (almostEqual(xi, xj) && almostEqual(yi, yj)) { // ignore small lines
        continue;
      }
  ...
  public static Bound rotatePolygon(NestPath polygon, int angle) {
    ...
    for (int i = 0; i < polygon.size(); i++) {
      double x = polygon.getSegment(i).x;
      double y = polygon.getSegment(i).y;
      double x1 = x * Math.cos(fAngle) - y * Math.sin(fAngle);
      double y1 = x * Math.sin(fAngle) + y * Math.cos(fAngle);
      rotated.add(x1, y1);
    }
    return getPolygonBounds(rotated);
  }
  ...
  public static NestPath rotatePolygon2Polygon(NestPath polygon, int degrees) {
    ...
    if (!polygon.getChildren().isEmpty()) {
  ...
  public static boolean isRectangle(NestPath poly, double tolerance) {
    ...
    for (int i = 0; i < poly.size(); i++) {
      if ((!almostEqual(poly.getSegment(i).x, bb.getXmin(), tolerance) && !almostEqual(poly.getSegment(i).x, bb.getXmin() + bb.getWidth(), tolerance)) || (!almostEqual(poly.getSegment(i).y, bb.getYmin(), tolerance) && !almostEqual(poly.getSegment(i).y, bb.getYmin() + bb.getHeight(), tolerance))) {
        return false;
      }
    }
    return true;
  ...
  public static List<NestPath> noFitPolygon(final NestPath A, final NestPath B, boolean inside, boolean searchEdges) {
    ...
    double minA = A.getSegment(0).y;
    int minAIndex = 0;
    double currentAX = A.getSegment(0).x;
    double maxB = B.getSegment(0).y;
    int maxBIndex = 0;

    for (int i = 1; i < A.size(); i++) {
      A.getSegment(i).marked = false;
      if (almostEqual(A.getSegment(i).y, minA) && A.getSegment(i).x < currentAX) {
        minA = A.getSegment(i).y;
        minAIndex = i;
        currentAX = A.getSegment(i).x;
      } else if (A.getSegment(i).y < minA) {
        minA = A.getSegment(i).y;
        minAIndex = i;
        currentAX = A.getSegment(i).x;
      }
    ...
    for (int i = 1; i < B.size(); i++) {
      B.getSegment(i).marked = false;
      if (B.getSegment(i).y > maxB) {
        maxB = B.getSegment(i).y;
        maxBIndex = i;
      }
    }
    Segment startPoint = null;
    if (!inside) {
      startPoint = new Segment(A.getSegment(minAIndex).x - B.getSegment(maxBIndex).x,
          A.getSegment(minAIndex).y - B.getSegment(maxBIndex).y);
    ...
    while (startPoint != null) {
      ...
      NFP.add(new Segment(B.getSegment(0).x + B.getOffsetX(),
          B.getSegment(0).y + B.getOffsetY()));

      double referenceX = B.getSegment(0).x + B.getOffsetX();
      double referenceY = B.getSegment(0).y + B.getOffsetY();
      ...
      while (counter < 10 * (A.size() + B.size())) {
        ...
        for (int i = 0; i < A.size(); i++) {
          ...
          for (int j = 0; j < B.size(); j++) {
            ...
            if (almostEqual(A.getSegment(i).x, B.getSegment(j).x + B.offsetX) && almostEqual(A.getSegment(i).y, B.getSegment(j).y + B.offsetY)) {
              touching.add(new SegmentRelation(0, i, j));
            } else if (onSegment(A.getSegment(i), A.getSegment(nexti), new Segment(B.getSegment(j).x + B.offsetX, B.getSegment(j).y + B.offsetY))) {
              touching.add(new SegmentRelation(1, nexti, j));
            } else if (onSegment(new Segment(B.getSegment(j).x + B.offsetX, B.getSegment(j).y + B.offsetY),
                new Segment(B.getSegment(nextj).x + B.offsetX, B.getSegment(nextj).y + B.offsetY),
                A.getSegment(i))) {
        ...
        for (SegmentRelation element : touching) {
          Segment vertexA = A.getSegment(element.A);
          vertexA.marked = true;
          ...
          Segment prevA = A.getSegment(prevAIndex);
          Segment nextA = A.getSegment(nextAIndex);

          Segment vertexB = B.getSegment(element.B);
          ...
          Segment prevB = B.getSegment(prevBIndex);
          Segment nextB = B.getSegment(nextBIndex);
        ...
        for (int i = 0; i < vectors.size(); i++) {
          if (almostEqual(vectors.getSegment(i).x, 0) && almostEqual(vectors.getSegment(i).y, 0)) {
            continue;
          }

          if (prevvector != null && vectors.getSegment(i).y * prevvector.y + vectors.getSegment(i).x * prevvector.x < 0) {

            double vectorlength = Math.sqrt(vectors.getSegment(i).x * vectors.getSegment(i).x + vectors.getSegment(i).y * vectors.getSegment(i).y);
            Segment unitv = new Segment(vectors.getSegment(i).x / vectorlength, vectors.getSegment(i).y / vectorlength);
          ...
          Double d = polygonSlideDistance(A, B, vectors.getSegment(i), true);

          double vecd2 = vectors.getSegment(i).x * vectors.getSegment(i).x + vectors.getSegment(i).y * vectors.getSegment(i).y;

          if (d == null || d * d > vecd2) {
            double vecd = Math.sqrt(vectors.getSegment(i).x * vectors.getSegment(i).x + vectors.getSegment(i).y * vectors.getSegment(i).y);
            d = vecd;
          }

          if (d != null && d > maxd) {
            maxd = d;
            translate = vectors.getSegment(i);
          }
        ...
        if (NFP.size() > 0) {
          for (int i = 0; i < NFP.size() - 1; i++) {
            if (almostEqual(referenceX, NFP.getSegment(i).x) && almostEqual(referenceY, NFP.getSegment(i).y)) {
              looped = true;
            }
          }
        }
  ...
  public static Segment searchStartPoint(NestPath CA, NestPath CB, boolean inside, List<NestPath> NFP) {
    ...
    if (A.getSegment(0) != A.getSegment(A.size() - 1)) {
      A.add(A.getSegment(0));
    }

    if (B.getSegment(0) != B.getSegment(B.size() - 1)) {
      B.add(B.getSegment(0));
    }

    for (int i = 0; i < A.size() - 1; i++) {
      if (!A.getSegment(i).marked) {
        A.getSegment(i).marked = true;
        for (int j = 0; j < B.size(); j++) {
          B.offsetX = A.getSegment(i).x - B.getSegment(j).x;
          B.offsetY = A.getSegment(i).y - B.getSegment(j).y;
          Boolean Binside = null;
          for (int k = 0; k < B.size(); k++) {
            Boolean inpoly = pointInPolygon(new Segment(B.getSegment(k).x + B.offsetX, B.getSegment(k).y + B.offsetY), A);
            if (inpoly != null) {
              Binside = inpoly;
              break;
            }
          ...
          double vx = A.getSegment(i + 1).x - A.getSegment(i).x;
          double vy = A.getSegment(i + 1).y - A.getSegment(i).y;
  ...
  private static boolean inNfp(Segment p, List<NestPath> nfp) {
    ...
    for (NestPath element : nfp) {
      for (int j = 0; j < element.size(); j++) {
        if (almostEqual(p.x, element.getSegment(j).x) && almostEqual(p.y, element.getSegment(j).y)) {
          return true;
        }
      }
    }
  ...
  private static Double polygonProjectionDistance(NestPath CA, NestPath CB, Segment direction) {
    ...
    for (int i = 0; i < edgeB.size(); i++) {
      ...
      for (int j = 0; j < edgeA.size() - 1; j++) {
        p = new Segment(edgeB.getSegment(i).x + Boffsetx, edgeB.getSegment(i).y + Boffsety);
        s1 = new Segment(edgeA.getSegment(j).x + Aoffsetx, edgeA.getSegment(j).y + Aoffsety);
        s2 = new Segment(edgeA.getSegment(j + 1).x + Aoffsetx, edgeA.getSegment(j + 1).y + Aoffsety);
  ...
  public static boolean intersect(final NestPath CA, final NestPath CB) {
    ...
    for (int i = 0; i < A.size() - 1; i++) {
      for (int j = 0; j < B.size() - 1; j++) {
        Segment a1 = new Segment(A.getSegment(i).x + Aoffsetx, A.getSegment(i).y + Aoffsety);
        Segment a2 = new Segment(A.getSegment(i + 1).x + Aoffsetx, A.getSegment(i + 1).y + Aoffsety);
        Segment b1 = new Segment(B.getSegment(j).x + Boffsetx, B.getSegment(j).y + Boffsety);
        Segment b2 = new Segment(B.getSegment(j + 1).x + Boffsetx, B.getSegment(j + 1).y + Boffsety);
        ...
        if (B.getSegment(prevbindex) == B.getSegment(j) || (almostEqual(B.getSegment(prevbindex).x, B.getSegment(j).x) && almostEqual(B.getSegment(prevbindex).y, B.getSegment(j).y))) {
          prevbindex = (prevbindex == 0) ? B.size() - 1 : prevbindex - 1;
        }

        if (A.getSegment(prevaindex) == A.getSegment(i) || (almostEqual(A.getSegment(prevaindex).x, A.getSegment(i).x) && almostEqual(A.getSegment(prevaindex).y, A.getSegment(i).y))) {
          prevaindex = (prevaindex == 0) ? A.size() - 1 : prevaindex - 1;
        }

        // go even further forward if we happen to hit on a loop end point
        if (B.getSegment(nextbindex) == B.getSegment(j + 1) || (almostEqual(B.getSegment(nextbindex).x, B.getSegment(j + 1).x) && almostEqual(B.getSegment(nextbindex).y, B.getSegment(j + 1).y))) {
          nextbindex = (nextbindex == B.size() - 1) ? 0 : nextbindex + 1;
        }

        if (A.getSegment(nextaindex) == A.getSegment(i + 1) || (almostEqual(A.getSegment(nextaindex).x, A.getSegment(i + 1).x) && almostEqual(A.getSegment(nextaindex).y, A.getSegment(i + 1).y))) {
          nextaindex = (nextaindex == A.size() - 1) ? 0 : nextaindex + 1;
        }

        Segment a0 = new Segment(A.getSegment(prevaindex).x + Aoffsetx, A.getSegment(prevaindex).y + Aoffsety);
        Segment b0 = new Segment(B.getSegment(prevbindex).x + Boffsetx, B.getSegment(prevbindex).y + Boffsety);

        Segment a3 = new Segment(A.getSegment(nextaindex).x + Aoffsetx, A.getSegment(nextaindex).y + Aoffsety);
        Segment b3 = new Segment(B.getSegment(nextbindex).x + Boffsetx, B.getSegment(nextbindex).y + Boffsety);
...
//            System.out.println(" not infi ");
  ...
  static Double polygonSlideDistance(final NestPath TA, final NestPath TB, Segment direction, boolean ignoreNegative) {
    ...
    for (int i = 0; i < edgeB.size() - 1; i++) {
      for (int j = 0; j < edgeA.size() - 1; j++) {
        A1 = new Segment(edgeA.getSegment(j).x + Aoffsetx, edgeA.getSegment(j).y + Aoffsety);
        A2 = new Segment(edgeA.getSegment(j + 1).x + Aoffsetx, edgeA.getSegment(j + 1).y + Aoffsety);
        B1 = new Segment(edgeB.getSegment(i).x + Boffsetx, edgeB.getSegment(i).y + BoffsetY);
        B2 = new Segment(edgeB.getSegment(i + 1).x + Boffsetx, edgeB.getSegment(i + 1).y + BoffsetY);
  ...
  public static List<NestPath> noFitPolygonRectangle(NestPath A, NestPath B) {
    double minAx = A.getSegment(0).x;
    double minAy = A.getSegment(0).y;
    double maxAx = A.getSegment(0).x;
    double maxAy = A.getSegment(0).y;

    for (int i = 1; i < A.size(); i++) {
      if (A.getSegment(i).x < minAx) {
        minAx = A.getSegment(i).x;
      }
      if (A.getSegment(i).y < minAy) {
        minAy = A.getSegment(i).y;
      }
      if (A.getSegment(i).x > maxAx) {
        maxAx = A.getSegment(i).x;
      }
      if (A.getSegment(i).y > maxAy) {
        maxAy = A.getSegment(i).y;
      }
    }

    double minBx = B.getSegment(0).x;
    double minBy = B.getSegment(0).y;
    double maxBx = B.getSegment(0).x;
    double maxBy = B.getSegment(0).y;
    for (int i = 1; i < B.size(); i++) {
      if (B.getSegment(i).x < minBx) {
        minBx = B.getSegment(i).x;
      }
      if (B.getSegment(i).y < minBy) {
        minBy = B.getSegment(i).y;
      }
      if (B.getSegment(i).x > maxBx) {
        maxBx = B.getSegment(i).x;
      }
      if (B.getSegment(i).y > maxBy) {
        maxBy = B.getSegment(i).y;
      }
    }
    ...
    res.add(minAx - minBx + B.getSegment(0).x, minAy - minBy + B.getSegment(0).y);
    res.add(maxAx - maxBx + B.getSegment(0).x, minAy - minBy + B.getSegment(0).y);
    res.add(maxAx - maxBx + B.getSegment(0).x, maxAy - maxBy + B.getSegment(0).y);
    res.add(minAx - minBx + B.getSegment(0).x, maxAy - maxBy + B.getSegment(0).y);
    nfpRect.add(res);
    return nfpRect;
  }
  ...
  public static List<NestPath> minkowskiDifference(NestPath A, NestPath B) {
    ...
    for (int i = 0; i < clipperNfp.size(); i++) {
      clipperNfp.getSegment(i).x += B.getSegment(0).x;
      clipperNfp.getSegment(i).y += B.getSegment(0).y;
    }
