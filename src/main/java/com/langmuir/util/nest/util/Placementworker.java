...
import com.langmuir.util.nest.data.NfpKey;
import com.langmuir.util.nest.data.PathPlacement;
import com.langmuir.util.nest.data.Result;
import com.langmuir.util.nest.data.Segment;
import com.langmuir.util.nest.util.coor.ClipperCoor;
        ...
        if (placed.size() == 0) {
          ...
          for (NestPath element : binNfp) {
            for (int k = 0; k < element.size(); k++) {
              if (position == null || element.getSegment(k).x - path.getSegment(0).x < position.x) {
                position = new PathPlacement(
                    element.getSegment(k).x - path.getSegment(0).x,
                    element.getSegment(k).y - path.getSegment(0).y,
                    path.getId(),
                    path.getRotation()
                );
              }
          ...
          for (int k = 0; k < nf.size(); k++) {
            ...
            for (int m = 0; m < placed.size(); m++) {
              for (int n = 0; n < placed.get(m).size(); n++) {
                allpoints.add(new Segment(placed.get(m).getSegment(n).x + placements.get(m).x,
                    placed.get(m).getSegment(n).y + placements.get(m).y));
              }
            }
            shifvector = new PathPlacement(
                nf.getSegment(k).x - path.getSegment(0).x,
                nf.getSegment(k).y - path.getSegment(0).y,
                path.getId(),
                path.getRotation(),
                combinedNfp
            );
            for (int m = 0; m < path.size(); m++) {
              allpoints.add(new Segment(path.getSegment(m).x + shifvector.x, path.getSegment(m).y + shifvector.y));
            }
